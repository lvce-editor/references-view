...
export const getReferences2 = async (uri: string, languageId: string, offset: number, position: any): Promise<readonly Reference[]> => {
  const references = await ExtensionHostReference.executeReferenceProvider2(uri, languageId, offset, position)
  const withPreview = await GetReferencesWithPreview.getReferencesWithPreview(references)
  return withPreview
}
