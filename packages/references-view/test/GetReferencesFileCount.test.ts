...
test('getFileCount should count unique URIs', () => {
  const references: readonly { readonly uri: string }[] = [{ uri: 'file1.ts' }, { uri: 'file2.ts' }, { uri: 'file1.ts' }, { uri: 'file3.ts' }]
...
test('getFileCount should return 1 for single file', () => {
  const references: readonly { readonly uri: string }[] = [{ uri: 'file1.ts' }, { uri: 'file1.ts' }]
